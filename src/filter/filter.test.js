...
it('filter([1,2,3], (item) => item) should retrun [1,2,3]', () => {
  expect(filter([1,2,3], (item) => item)).to.be.deep.equal([1,2,3]);
});

it('filter([0,1,2], (item) => item) should retrun [1,2]', () => {
  expect(filter([0,1,2], (item) => item)).to.be.deep.equal([1,2]);
});

it('filter([1,2,3], (item) => item < 2) should retrun [1]', () => {
  expect(filter([1,2,3], (item) => item < 2)).to.be.deep.equal([1]);
});

it('filter([1,2,3, 5], (item,index) => item === index + 1) should retrun [1,2,3]', () => {
  expect(filter([1,2,3,5], (item, index) => item === index + 1)).to.be.deep.equal([1,2,3]);
});

it('filter([1,2,3, 5, 2, 1], (item,index, array) => index === array.indexOf(item)) should retrun [1,2,3,5]', () => {
